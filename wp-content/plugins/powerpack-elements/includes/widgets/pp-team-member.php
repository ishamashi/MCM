 ...
 * Team Member Widget
 */
class PP_Team_Member_Widget extends Widget_Base {
    ...
    protected function render_name() {
        ...
        $this->add_render_attribute( 'team_member_name', 'class', 'pp-tm-name' );

        if ( $settings['team_member_name'] != '' ) {
            printf( '<%1$s %2$s>%3$s</%1$s>', $settings['name_html_tag'], $this->get_render_attribute_string( 'team_member_name' ), $settings['team_member_name'] );
        }
...
Plugin::instance()->widgets_manager->register_widget_type( new PP_Team_Member_Widget() );