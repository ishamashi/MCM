...
2. Unzip the downloaded addons-for-elementor-pro.zip file and upload to the `/wp-content/plugins/` directory or install the Elementor Addons by Livemesh plugin from WordPress repository. Activate the plugin through the 'Plugins' menu in WordPress.
...
= 1.5 =
* Added - Moved the Livemesh Grid widget to new framework based on reusable blocks. Pls note that the class names have changed to keep it consistent with the new framework.
* Added - 6 different styles for grid items plus 7 header styles too
* Added - Next Prev pagination options for Livemesh Grid widget
...
* Credit - Big thanks to Axel for finding ALL these bugs that I could not have found myself.
