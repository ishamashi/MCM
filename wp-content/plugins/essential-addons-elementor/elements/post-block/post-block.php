...
class Widget_Eael_Post_Block extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
            'eael_post_block_show_load_more',
            [
        ...
        $this->add_control(
			'eael_post_block_show_load_more_text',
			[
				...
				'condition' => [
					'eael_post_block_show_load_more' => '1',
				]
			]
		);
		...
		$this->add_responsive_control(
			'eael_post_block_thumb_image_height',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .eael-post-block-item .eael-entry-thumbnail' => 'height: {{SIZE}}px;',
				],
			]
		);
		...
		$this->start_controls_tabs( 'eael_post_block_load_more_btn_tabs' );
			...
			$this->add_group_control(
				Group_Control_Box_Shadow::get_type(),
				[
					'name' => 'eael_post_block_load_more_btn_hover_shadow',
	...
	protected function render( ) {
		...
		<?php if( 1 == $settings['eael_post_block_show_load_more'] ) : ?>
		<!-- Load More Button -->
		<div class="eael-load-more-button-wrap">
			<button class="eael-load-more-button" id="eael-load-more-btn-<?php echo $this->get_id(); ?>">
				<div class="eael-btn-loader button__loader"></div>
		  		<span><?php echo esc_html__( $settings['eael_post_block_show_load_more_text'], 'essential-addons-elementor' ); ?></span>
			</button>
		</div>
...
jQuery(document).ready(function($) {
	...
	var settings = {
		...
		metaPosition: '<?php echo $settings['eael_post_block_meta_position']; ?>',
		excerptLength: parseInt( <?php echo $settings['eael_excerpt_length']; ?>, 10 ),
		btnText: '<?php echo $settings['eael_post_block_show_load_more_text']; ?>',
