...
function essential_addons_el_enqueue(){
   ...
   if( $is_component_active['one-page-navigation'] ) {
    ...
    if( $is_component_active['team-member-carousel'] || $is_component_active['logo-carousel']) {
        wp_enqueue_script('essential_addons_elementor-swiper-carousel-js',ESSENTIAL_ADDONS_EL_URL.'assets/js/swiper.jquery.min.js', array('jquery'),'1.0', true);
    }
    if( $is_component_active['image-hotspots'] ) {
