...
class Aepro_Custom_Field extends Widget_Base {
	...
	protected function render()
	{
		...
		if(class_exists('acf') && in_array($settings['cf_type'],['text','link','audio']) && $settings['acf_support'] == 'yes'){
		...
		switch ($cf_type) {
			...
			case "link":	if($settings['link_type'] == 'email'){
                            ...
                            if(!empty($settings['cf_link_text']) && $settings['link_text_type'] == 'static'){
                                ...
                                $custom_field_html = '<a '.$this->get_render_attribute_string( 'custom-field-class' ).'  href="'.$custom_field_val.'">'.get_post_meta($post_id,$settings['cf_link_dynamic_text'], true).'</a>';
                            }else{
